...
- The flip is the page's set-piece. The full motion inventory (as shipped 2026-07-21, supersedes earlier drafts of this bullet): (1) **Close cover-slip** — subordinate scripted moment; a content-width slip of chapter ink reading "Where Do We Begin?" holds 0.6s and slides off right (1.4s power3.inOut, once, slight text drag); `visibility:hidden` in CSS so the static page is complete. (2) **Foundation beam fill** — reveal-tier; each heading word carries an accent layer clipped shut, the beam opens it left-to-right and drains behind (clip-path only); the em word fills and holds, and its fill is statically open so the resting document shows the end state. (3) **Hero jobs ticking** — ambient; the three snapshot sentences' marks ignite in sequence, dim, and rest (~7.8s cycle), running only while the hero is in view; marks reserve width always and are invisible in the static document. All three live inside the reduced-motion guard. Nothing else gets choreography. No bounce/elastic; no layout-property animation.
