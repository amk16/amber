...
- The flip is the page's set-piece. One subordinate scripted moment (amendment 2026-07-21): the **Close cover-slip**, a slip of chapter ink over the ask's header reading "Where Do We Begin?" (content-width, no bleed): it holds a beat, the question lifts, and the ink — sliced into 10 vertical strips — pulls upward at staggered speeds (0.9–1.4s, each 0.07 from edges, power3.inOut, once), the reveal filling like a cup. Flip tokens only; `visibility:hidden` in CSS so the static page is complete. One reveal-tier flourish (2026-07-21, cup-fill form): the **Foundation ink pass** — each heading letter carries a statically-clipped accent twin that fills bottom-up like a cup at staggered speeds, accumulating left to right (no dark trailing edge); when the line is full the free letters release together and only the em word keeps the ink. Under reduced motion nothing runs and the resting document is the end state. Nothing else gets choreography. No bounce/elastic; no layout-property animation.
