...
- The flip is the page's set-piece. The choreography ledger (as shipped 2026-07-21): (1) **Close cover-slip** — content-width slip of chapter ink reading "Where Do We Begin?", holds 0.6s, slides off right 1.4s power3.inOut, once; (2) **Foundation beam fill** — a beam of accent moves rightward through the heading via clip-path, filling each word and draining behind; the em word fills and holds (its fill statically open = the resting state); (3) **Hero carriage** — on load the kicker types itself (per-char reveal, caret riding the writing edge, ~0.045s/char), the headline period stamps, the accent word underlines; caret stays blinking and stamp+underline loop every ~7s. All three dormant in static CSS: under reduced motion or no JS the finished document simply stands. Nothing else gets choreography. No bounce/elastic; no layout-property animation.
